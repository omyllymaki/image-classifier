...
class BaseImageData:
    ...
    def convert_labels_to_integers(self):
        ...
        for item in self.image_data:
            x = item['x']
            y = self.label_to_integer(item['y'])
    ...
    def label_to_integer(self, label):
        raise NotImplementedError
...
class SingleLabelImageData(BaseImageData):
    ...
    def __init__(self,
        ...
        super().__init__(image_data, p_training, p_valid, p_test)

    def get_unique_labels(self):
        return list(set([sample['y'] for sample in self.image_data]))

    def label_to_integer(self, label):
        return self.label_to_class_mapping[label]

    def _get_label(self, data, index):
        return self.class_to_label_mapping[data[index]['y']]


class MultiLabelImageData(BaseImageData):
